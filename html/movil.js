...
function gradeExercise(tarea, pregunta) {
    ...
    carga("https://elisa.dyndns-web.com/cgi-bin/verif.py?hash=" + hash + responses, "movil.html");
}
...
function fillValues(next) {
	...
	loadData("data/" + hash + ".awarded.xml", true, 4, next);
        submitbutton = "<button type='button' style='height:50px;width:100px;;background-color:#bbb891;' onclick='gradeExercise()'>Calificar</button>";
...
function greetUser() {
    var loggedIn = "<span style='color:#dc965a'>Usuario ";
    var logout = "<a href='movil.html' onclick='performLogout();''><strong>Salir</strong></a>"; 
...
function anonGreet() {
	...
	"<span style='color:#dc965a'>Haz <a onclick='performLogin();'><strong>login</strong></a>" +
	" para entregar tareas.</span>";
}

function performLogin() {
    student = prompt("Usuario:");
    if (student != null) {
